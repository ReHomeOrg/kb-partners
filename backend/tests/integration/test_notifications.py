...
async def test_emit_noop_when_disabled(session: AsyncSession) -> None:
    ...
    emit_notifications(
        session, request_id=uuid.uuid4(), number="RQ-1", status=RequestStatus.ASSIGNED
    )
...
async def test_emit_enqueues_per_audience(
    ...
    # CANCELLED уведомляет двух адресатов (заявитель + партнёр) → две строки.
    emit_notifications(
        session, request_id=uuid.uuid4(), number="RQ-1", status=RequestStatus.CANCELLED
    )
    await session.commit()
...
async def test_drain_marks_done(session: AsyncSession) -> None:
    ...
    await session.commit()
    processed = await drain_notification_batch(session, settings=Settings())
    assert processed == 1
...
async def test_drain_closes_broken_payload(session: AsyncSession) -> None:
    ...
    await session.commit()
    await drain_notification_batch(session, settings=Settings())
